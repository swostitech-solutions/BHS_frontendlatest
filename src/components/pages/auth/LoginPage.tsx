...
/////// added login with forgot password model and API call //////
...
const LoginPage = () => {
  ...
  // for forgot password  stage -1
  const [showForgotModal, setShowForgotModal] = useState(false);
  const [forgotStep, setForgotStep] = useState<1 | 2 | 3>(1);
  const [userIdentifier, setUserIdentifier] = useState("");
  const [identifierType] = useState("phone");
// stage - 2
  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="w-full max-w-md bg-slate-800 border border-slate-700 rounded-3xl shadow-2xl p-8 relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              disabled
              className="w-full bg-slate-700/30 border border-slate-600 rounded-xl px-4 py-3.5 text-slate-400"
            />
          </div>
          ...
          {/* <button
            ...
            className="w-full py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-indigo-500 to-violet-500 hover:opacity-90 transition-opacity"
          >
            Proceed
          </button> */}
          ...
          <button
            onClick={async () => {
              ...
              if (userIdentifier.length < 10) {
                setForgotError("Please enter valid phone number");
                return;
              }
              ...
              try {
                const res = await fetch(
                  `${API_BASE}/api/auth/forgot-password`,
                  {
                    ...
                    body: JSON.stringify({
                      identifier: userIdentifier,      // ✅ changed
            ...
            className="w-full py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-indigo-500 to-violet-500 hover:opacity-90 transition-opacity disabled:opacity-50"
          >
            {forgotLoading ? "Sending OTP..." : "Proceed"}
          </button>
  ...
  <div className="space-y-5">
    <div>
      ...
      <input
        ...
        maxLength={6}
        className="w-full bg-slate-700/50 border border-slate-600 rounded-xl px-4 py-3.5 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors text-center tracking-widest text-lg"
      />
    </div>
    ...
    {/* <button
      ...
      disabled={forgotLoading}
    >
      {forgotLoading ? "Verifying..." : "Proceed"}
    </button> */}
    ...
    <button
      onClick={async () => {
        ...
        try {
          ...
          setForgotError("Unable to verify OTP");
        } finally {
          setForgotLoading(false);
        }
      }}
        ...
        try {
          const res = await fetch(
            `${API_BASE}/api/auth/reset-password`,
            {
              ...
              body: JSON.stringify({
                resetToken: resetToken, // ✅ from stage 2
          ...
          setForgotError("Unable to reset password");
        } finally {
          setForgotLoading(false);
        }
      }}
