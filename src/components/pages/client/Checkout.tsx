...
//               {booking.subservice?.name || booking.subservice_code || "Service"}
//             </p>
...
useEffect(() => {
  const init = async () => {
    ...
    /* ================= PAYMENT SUCCESS ================= */
    if (paymentSuccess) {
      const clearFlag = sessionStorage.getItem("clearCartAfterPayment");

      if (clearFlag && userData) {
        try {
          const parsedUser = JSON.parse(userData);

          // ✅ Clear backend cart
          // await fetch(`${API_BASE}/api/cart/item/${parsedUser.id}`, {
          //   method: "DELETE",
          // });
...
for (const item of pendingCart) {
  try {
    ...
    console.error("Failed to delete item:", item.id);
        ...
        sessionStorage.removeItem("clearCartAfterPayment");
        sessionStorage.removeItem("cart");
        sessionStorage.removeItem("bookingData");
        sessionStorage.removeItem("pendingCart");
      }
  ...
  try {
    ...
    if (paymentMethod === "ONLINE") {
      try {
        ...
        // ✅ SAVE CART + FLAG for clearing after success
        sessionStorage.setItem("pendingCart", JSON.stringify(cart));
